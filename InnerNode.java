...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(BaseTransaction transaction, DataBox key) {
        ...
        BPlusNode child = getChild(transaction, idx);
    ...
    throws BPlusTreeException {
    	...
    	Optional<Pair<DataBox, Integer>> result = child.put(transaction, key, rid);
    	...
    	Pair<DataBox, Integer> prop = result.get();    	
    	keys.add(idx, prop.getFirst());
    	children.add(idx + 1, prop.getSecond());
    ...
    throws BPlusTreeException {
        ...
        while (data.hasNext() && keys.size() <= 2*d) {
          ...
          Optional<Pair<DataBox, Integer>> result = rightChild.bulkLoad(transaction, data, fillFactor);
          
          if (result.isPresent()) {
            Pair<DataBox, Integer> n = result.get();
            keys.add(keys.size(), n.getFirst());
            children.add(children.size(), n.getSecond());
          }
        }
